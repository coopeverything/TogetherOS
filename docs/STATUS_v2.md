...
| **Search & Tags** | Global search with filters, full-text search across proposals + forum topics + posts (Phase 1-2 ✅), cooperation path filtering, privacy-preserving query tracking, relevance scoring | <!-- progress:search=85 --> 85% | Expand to profiles/groups (Phase 3) | Phase 2 complete: Taxonomy linking ✅, Forum full-text ✅, Profiles/groups pending |
...
| Module | Scope (what it covers) | Progress | Next milestone | Blockers / Notes |
| --- | --- | ---:| --- | --- |
| **Proposals & Decisions** | Proposal object, evidence/options, vote, review, minority reports, amendment process, consent-based governance | <!-- progress:governance=75 --> 75% | Voting logic refinements + evidence UI | PostgreSQL repos complete ✅, API handlers complete ✅, UI CRUD complete ✅ |
...
| **Observability** | Self-hosted error/perf logging with PII sanitization, alert manager (Discord/Slack), Prometheus metrics API, monitoring UI, comprehensive test suite, Docker stack ready (Uptime Kuma, Grafana, Loki), APM distributed tracing, contract testing, feature flags, canary deployment | <!-- progress:observability=70 --> 70% | Activate Docker monitoring stack when needed | Phase 4 complete - canary deployment & feature flags |
