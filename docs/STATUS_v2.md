...
| **Support Points & Reputation** | Points bank, allocation per idea, badges, SP for governance power (only from contributions), RP for economic claims | <!-- progress:reputation=65 --> 65% | Full badge criteria evaluation from reward_events | Backend complete: DB + API + types + badge CRUD + ReputationService. Badge display integrated with RPWalletClient. |
