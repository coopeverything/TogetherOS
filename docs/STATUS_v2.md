...
| **Observability** | Self-hosted error/perf logging with PII sanitization, alert manager (Discord/Slack), Prometheus metrics API, monitoring UI, comprehensive test suite, Docker stack ready (Uptime Kuma, Grafana, Loki) | <!-- progress:observability=40 --> 40% | Activate Docker monitoring stack when needed | Phase 1.5 complete - lightweight monitoring active |
