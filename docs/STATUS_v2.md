...
| **Identity & Auth** | Sign up/in, sessions, roles, privacy (email/handle), email verification, password reset, Google OAuth | <!-- progress:auth=95 --> 95% | Document OAuth env vars | OAuth implementation complete, needs env var documentation |
...
| **Feed** | Social media feed + native posts, multi-dimensional reactions, topic filtering, post composer (Phase 1-2 complete), Bridge topic intelligence with AI suggestions, duplicate thread detection, topic-based feeds (Phase 3 complete) | <!-- progress:feed=60 --> 60% | Multi-dimensional rating and reputation (Phase 4-5) | Evidence repository, sentiment mapping |
