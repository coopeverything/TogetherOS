...
| **Observability** | Self-hosted error/perf logging with PII sanitization, alert manager (Discord/Slack), Prometheus metrics API, monitoring UI, comprehensive test suite, Docker stack ready (Uptime Kuma, Grafana, Loki), APM distributed tracing, contract testing | <!-- progress:observability=60 --> 60% | Activate Docker monitoring stack when needed | Phase 3 complete - APM & contract testing |
