    ...
    <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 ${className}`}>
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-between items-center">
          <span className="text-base text-gray-600 dark:text-gray-400">Your Balance</span>
          <span className="text-3xl font-bold text-emerald-600">{wallet.shBalance.toFixed(4)} SH</span>
        </div>

        <div className="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-3">
          {/* Total Issued */}
          <div className="flex justify-between items-center">
            <span className="text-base text-gray-700 dark:text-gray-300">Total Received</span>
            <span className="text-xl font-semibold text-gray-600 dark:text-gray-400">{wallet.totalIssued.toFixed(4)} SH</span>
          </div>

          {/* Ownership Percentage */}
          {ownershipPercentage !== undefined && (
            <div className="flex justify-between items-center">
              <span className="text-base text-gray-700 dark:text-gray-300">Ownership</span>
              <span className="text-xl font-semibold text-indigo-600">{ownershipPercentage.toFixed(4)}%</span>
            </div>
          )}
          ...
          {totalCirculation !== undefined && totalCirculation > 0 && (
            <div className="flex justify-between items-center">
              <span className="text-base text-gray-700 dark:text-gray-300">Total in Circulation</span>
              <span className="text-base text-gray-500 dark:text-gray-400">{totalCirculation.toFixed(2)} SH</span>
            </div>
          )}
        </div>
