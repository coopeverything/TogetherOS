...
export async function POST(request: NextRequest) {
  try {
    ...
    const { topic, archetypeId } = body

    if (!topic || !archetypeId) {
      return NextResponse.json(
        { error: 'topic and archetypeId are required' },
        { status: 400 }
      )
    }

    const session = await createTeachingSession(user.id, topic, archetypeId)
