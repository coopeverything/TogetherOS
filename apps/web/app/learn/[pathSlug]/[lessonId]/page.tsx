...
function StructuredContentView({ content }: { content: StructuredContent }) {
  ...
  return (
    <div className="space-y-6">
      {/* Introduction */}
      <div>
        <p className="text-gray-700 leading-relaxed">{structured.introduction}</p>
      </div>
        ...
        <div className="space-y-4">
          ...
          {video.transcript && (
            <details className="bg-gray-50 p-4 rounded-lg">
              ...
              <p className="mt-2 text-gray-600 whitespace-pre-wrap">{video.transcript}</p>
            </details>
          )}
        </div>
      )
  ...
  const renderMarkdown = (text: string) => {
    ...
    lines.forEach((line, i) => {
      ...
      if (trimmed.startsWith('### ')) {
        ...
        flushList()
        elements.push(
          <h2 key={i} className="text-xl font-semibold text-gray-900 mt-6 mb-3">
            {trimmed.slice(3)}
          </h2>
        )
      } else if (trimmed.startsWith('# ')) {
